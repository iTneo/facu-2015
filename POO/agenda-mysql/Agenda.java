...
public class Agenda {
	...
	public void consultar(){
		...
		ResultSet rs = null;
		try {
			PreparedStatement pst = (PreparedStatement) bo.conn.prepareStatement("select nome, endereco, telefone, email from agenda where nome like ?");
			nomeBusca = "%" + nomeBusca + "%";
	...
	public void remover() {
		System.out.println("Digite um nome");
		String nomeDeleta = entrada.next();
		try {
			PreparedStatement pst = (PreparedStatement) bo.conn
					.prepareStatement("delete from agenda where nome = ?");
			pst.setString(1, nomeDeleta);
			pst.execute();
	...
	public void editar() {
		...
		System.out.println("Digite um nome para alterar esse contato");
